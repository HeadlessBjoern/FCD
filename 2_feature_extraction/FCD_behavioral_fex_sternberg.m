%% FCD Behavioral Feature Extraction Sternberg
% 
% Extracted features:
%   Accuracy
%   Reaction Time
...
clc
close all
path = '/Volumes/methlab_data/FCD/data/';
dirs = dir(path);
...
for subj = 1:length(subjects)
    ...
    trial_counter = 1;
    for block = 1:6
        load(strcat(subjects{subj}, '_FCD_Sternberg_block', num2str(block), '_task.mat'))
    ...
    %% Save
    savepath = strcat('/Volumes/methlab/Students/Arne/FCD/data/features/',subjects{subj}, '/behavioral/');
    mkdir(savepath)
    cd(savepath)
...
save /Volumes/methlab/Students/Arne/FCD/data/features/behavioral_matrix_sternberg behav_data_sternberg